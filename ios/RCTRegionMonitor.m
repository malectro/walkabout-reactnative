...
#import "RCTRegionMonitor.h"

#import <CoreLocation/CoreLocation.h>
#import <React/RCTLog.h>

NSString* RCTRegionMonitorDidChangeRegionEvent = @"RCTRegionMonitorDidChangeRegionEvent";
NSString* RCTRegionMonitorErrorDomain = @"RCTRegionMonitorErrorDomain";

@implementation RCTRegionMonitor
...
{
    return @[RCTRegionMonitorDidChangeRegionEvent];
}

- (NSDictionary *)constantsToExport
{
    return @{
        @"regionMonitorDidChangeRegion": RCTRegionMonitorDidChangeRegionEvent,
    };
}
...
- (void)_sendQueuedRegionEvents {
    for (NSDictionary *body in queuedRegionEvents) {
        [self sendEventWithName:RCTRegionMonitorDidChangeRegionEvent body:body];
    }
...
- (void)_requestAuthorization {
    NSString *plistKey = @"NSLocationWhenInUseUsageDescription";
    NSString *alwaysUsageDescription = [NSBundle mainBundle].infoDictionary[plistKey];

    if (!alwaysUsageDescription) {
        NSError *error = [[NSError alloc] initWithDomain:RCTRegionMonitorErrorDomain code:9 userInfo:@{
            @"status": @0,
...
- (CLLocationCoordinate2D)_parseCoordinateDictionary:(NSDictionary *)coordinate {
  CLLocationCoordinate2D clCoordinate;
  clCoordinate.latitude = [coordinate[@"latitude"] doubleValue];
  clCoordinate.longitude = [coordinate[@"longitude"] doubleValue];
  return clCoordinate;
}
...
- (void)_addCircularRegion:(NSDictionary *)center
    ...
    CLLocationCoordinate2D coordinate = [self _parseCoordinateDictionary:center];
...
RCT_EXPORT_METHOD(addCircularRegion:(nonnull NSDictionary *)center
    ...
    if (![self _isRegionMonitoringPossible:status]) {
        RCTLogInfo(@"status %d", status);
        NSError *error = [[NSError alloc] initWithDomain:RCTRegionMonitorErrorDomain code:6 userInfo:nil];
    ...
    if (pendingRegions[identifier]) {
        ...
        NSError *error = [[NSError alloc] initWithDomain:RCTRegionMonitorErrorDomain code:1 userInfo:nil];
    ...
    if (existingRegion != nil) {
        NSError *error = [[NSError alloc] initWithDomain:RCTRegionMonitorErrorDomain code:8 userInfo:nil];
...
RCT_EXPORT_METHOD(removeCircularRegion:(nonnull NSString *)identifier
    ...
    if (region != nil) {
        ...
        NSError *error = [[NSError alloc] initWithDomain:RCTRegionMonitorErrorDomain code:2 userInfo:nil];
        reject(@"remove_region", @"Removing region failed because the region does not exist.", error);
    }
}

RCT_EXPORT_METHOD(checkRegion:(nonnull NSString *)identifier
                  coordinate:(nonnull NSDictionary *)coordinate
                  resolver:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject) {
  CLLocationCoordinate2D clCoordinate = [self _parseCoordinateDictionary:coordinate];
  CLCircularRegion *region = [self _getMonitoredRegionWithIdentifier:identifier];
  ...
  if (region == nil) {
    ...
    NSError *error = [[NSError alloc] initWithDomain:RCTRegionMonitorErrorDomain code:2 userInfo:nil];
    reject(@"check_region", @"Checking region for coordinate failed because the region does not exist.", error);
  } else {
    resolve(@([region containsCoordinate:clCoordinate]));
  }
}

- (NSArray<NSString *> *)_convertRegionsToIdentifiers:(NSSet<CLRegion *> *)regionsSet {
  NSMutableArray<NSString *> *regions = [NSMutableArray arrayWithCapacity:regionsSet.count];
  for (CLRegion * region in regionsSet) {
    [regions addObject:region.identifier];
  }
  return regions;
}

RCT_EXPORT_METHOD(monitoredRegions:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject) {
  resolve(
          [self _convertRegionsToIdentifiers:locationManager.monitoredRegions]
          );
}

RCT_EXPORT_METHOD(checkRegionsForCoordinate:(nonnull NSDictionary *)coordinate
                  resolver:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject) {
  CLLocationCoordinate2D clCoordinate = [self _parseCoordinateDictionary:coordinate];
  NSSet *regions = [locationManager.monitoredRegions objectsPassingTest:^BOOL(CLCircularRegion *region, BOOL *stop) {
    return ([region containsCoordinate:clCoordinate]);
  }];
  resolve([self _convertRegionsToIdentifiers:regions]);
}
...
RCT_EXPORT_METHOD(requestAuthorization:(RCTPromiseResolveBlock)resolve
    ...
    if ([self _isRegionMonitoringPossible:status]) {
        ...
        NSError *error = [[NSError alloc] initWithDomain:RCTRegionMonitorErrorDomain code:6 userInfo:@{
            @"status": @(status),
        }];
...
- (void)locationManager:(CLLocationManager *)manager didChangeAuthorizationStatus:(CLAuthorizationStatus)status {
    ...
    if (status == kCLAuthorizationStatusAuthorizedAlways ||
        ...
        NSError *error = [[NSError alloc] initWithDomain:RCTRegionMonitorErrorDomain code:5 userInfo:@{
            @"status": @(status),
        }];
